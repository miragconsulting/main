...
const cards = fs.readdirSync(baseDir)
  ...
  .map(folder => {
    ...
    $('img').each((_, el) => {
      ...
      if (!image && /\/sea_house\/\d+\/\d+\.(jpe?g|png)$/i.test(src)) image = src;
    });
    if (!image)
...
indexHtml = indexHtml.replace(
  /<!-- SECTION: Продажа: Дома у моря -->[\s\S]*?<\/section>/i,
`<!-- SECTION: Продажа: Дома у моря -->
...
console.log('✅ Блок «Дома у моря» обновлён: кнопки получили такие же градиенты, как в галерее проектов.');
